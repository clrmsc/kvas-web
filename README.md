...
>
> ### Перенос на другой роутер
>
> Поставьте пакет как обычно, выполните `kvas setup`, задайте пароль в
> браузере — и обновите клиент xray (**Настройки → Клиент xray**), иначе
> часть серверов подписки работать не будет.
>
> Настройки переносятся файлами. На старом роутере:
>
> ```sh
> tar czf /tmp/kvas-настройки.tar.gz /opt/etc/kvas.list \
>     /opt/etc/kvas-web/networks.list /opt/etc/kvas-web/subscription.json
> ```
>
> На новом — распаковать в корень, затем `kvas init` и
> `/opt/etc/init.d/S99kvasweb restart`. Пароль веб-интерфейса переносить
> не нужно: проще задать заново.
>
> ### Обновления
>
