...
# Метка версии выкладывается в релиз рядом с пакетами: по ней веб-интерфейс
# на роутере понимает, что вышла новая сборка. Версия закодирована в имени
# файла, а не в содержимом: раздача релизов какое-то время отдаёт прежнее
# содержимое по тому же имени, а список файлов через API всегда актуален.
# Тег релиза для этого не годится — он остаётся прежним.
mkdir -p "${OUT_DIR}"
rm -f "${OUT_DIR}"/version-*
# Содержимое дублирует версию: пустые файлы к релизу не прикладываются.
printf '%s-%s\n' "${PKG_VERSION}" "${PKG_RELEASE}" > "${OUT_DIR}/version-${PKG_VERSION}-${PKG_RELEASE}"
echo "${OUT_DIR}/version-${PKG_VERSION}-${PKG_RELEASE}"
